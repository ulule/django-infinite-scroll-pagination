    ...
    def create_articles(self):
        ...
        for i in range(25):
            Article.objects.create(
                title='%s' % i,
                date=date,
                date_unique=date + datetime.timedelta(minutes=i))

        return Article.objects.all()

    def create_articles_custom_pk(self):
        for i in range(25):
            ArticleCustomPK.objects.create(
                title='%s' % i,
                date=timezone.now() + datetime.timedelta(minutes=i))

        return Article.objects.all()


# -----------------------------------------------------------------------------
# Pagination order by PK
# -----------------------------------------------------------------------------

class PaginatorTest(TestBase):
    @before
    def before(self):
        self.create_articles()

        self.paginator = SeekPaginator(Article.objects.all(),
                                       per_page=10,
                                       lookup_field='date_unique',
                                       order_by_pk=True)

    def test_prepare_order(self):
        self.assertListEqual(sorted(self.paginator.prepare_order()),
                             sorted(['-date_unique', '-pk']))
...
class PaginatorViewTest(TestBase):
    @before
    def before(self):
        self.create_articles()
    ...
    def test_first_page(self):
        response = self.client.get(reverse('pagination-ajax'), HTTP_X_REQUESTED_WITH='XMLHttpRequest')
...
# -----------------------------------------------------------------------------
# Pagination with custom PK
# -----------------------------------------------------------------------------

class PaginatorCustomPKTest(TestBase):
    @before
    def before(self):
        self.create_articles_custom_pk()

        self.paginator = SeekPaginator(ArticleCustomPK.objects.all(),
                                       per_page=10,
                                       lookup_field='date',
                                       order_by_pk=False)
    ...
    def test_prepare_lookup(self):
        lookup_f, lookup_e = self.paginator.prepare_lookup(value=1)
        self.assertDictEqual(lookup_f, {'date__lte': 1, })
        self.assertDictEqual(lookup_e, {'date': 1})

    def test_paginator(self):
        articles = ArticleCustomPK.objects.all().order_by('-date')
        ...
        page_3 = self.paginator.page(value=page_2[-1].date, pk=page_2[-1].pk)
        self.assertListEqual(list(page_3), list(articles[20:]))

    def test_first_page(self):
        self.assertTrue(self.paginator.page().has_next())

    def test_last_page(self):
        articles = ArticleCustomPK.objects.all().order_by('-date')
        next_to_last = list(articles)[-2]
        page = self.paginator.page(value=next_to_last.date, pk=next_to_last.pk)
        self.assertFalse(page.has_next())


class PageCustomPKTest(TestBase):
    @before
    def before(self):
        self.create_articles_custom_pk()
    ...
    def test_objects_left(self):
        ...
        # last page
        art = list(articles)[-self.paginator.per_page]
        page_last = self.paginator.page(value=art.date, pk=art.pk)
        self.assertEqual(page_last.objects_left, 0)
